...
class CustomSegmentationHead(nn.Module):
    def __init__(self, in_channels, num_classes, task='binary', dropout_ratio=0.):
        ...
        self.conv3 = nn.Conv2d(in_channels // 4, num_classes, kernel_size=1)
    ...
    def forward(self, x, input_size, task='binary'):
        ...
        x = self.mish2(x)
        x = self.conv3(x)
        x = self.sigmoid(x) if task == 'binary' else self.softmax(x)
        return x

class ViTASeg(nn.Module):
    def __init__(self, img_size=224, num_classes=1, task='binary'):
        super(ViTASeg, self).__init__()
        self.vit = ViTAEv2(img_size=img_size)
        num_channels = self.vit(torch.zeros(1, 3, img_size, img_size))[0].shape[1]
        self.segmentation_head = CustomSegmentationHead(in_channels=num_channels, num_classes=num_classes, task=task)
        self.img_size = img_size

    def forward(self, x):
        x = self.vit(x)
        x = self.segmentation_head(x[0], input_size=(self.img_size, self.img_size))
        return x
        
""" model = ViTASeg(img_size=256)
