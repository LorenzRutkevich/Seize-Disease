...
from utils import binary_fastrcnn_loss, eval_forward
...
def create_model(num_classes: int , pretrained: bool = True, 
                coco_model: bool = False, bin_loss: bool = False, vit_dict: dict = None):
    if bin_loss:
        roi_heads.fastrcnn_loss = binary_fastrcnn_loss
    # Load the backbone.
    model_backbone = mobilevit_xxs(pretrained=pretrained) if vit_dict is None else mobilevit_xxs(pretrained=pretrained, **vit_dict)
...
if __name__ == '__main__':
    ...
    model = create_model(num_classes=1, pretrained=True, coco_model=True, bin_loss=True)
    try:
        summary(model)
    except:
        print(model)
    ...
    print(type(targets) for targets in out)
    losses, detections = eval_forward(model, dummy_input, out, bin_loss=True)
    print(losses, detections)
