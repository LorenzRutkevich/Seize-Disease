...
# https://github.com/Mr-TalhaIlyas/Prerpcessing-PanNuke-Nuclei-Instance-Segmentation-Dataset/tree/master
# @Mr-TalhaIlyas https://github.com/Mr-TalhaIlyas

data_dir = "/path/to/fold_x"  # location to extracted folds
output_dir = "path/to/save_dir"  # location to save op data
...
def get_boundaries(raw_mask):
    ...
    for i in range(raw_mask.shape[-1] - 1):  # because last chnnel is background
        bdr[:, :, i] = find_boundaries(
            raw_mask[:, :, i], connectivity=1, mode="thick", background=0
        )
...
for i, j in enumerate(folds):
    ...
    for k in trange(
        images.shape[0], desc="Writing files for {}".format(j), total=images.shape[0]
    ):
        ...
        Image.fromarray(sem_mask).save(
            output_dir
            + "/{}/sem_masks/sem_{}_{}_{:05d}.png".format(j, tissue_type, i + 1, k)
        )
        Image.fromarray(instances).save(
            output_dir
            + "/{}/inst_masks/inst_{}_{}_{:05d}.png".format(j, tissue_type, i + 1, k)
        )
        Image.fromarray(raw_image).save(
            output_dir
            + "/{}/images/img_{}_{}_{:05d}.png".format(j, tissue_type, i + 1, k)
        )
